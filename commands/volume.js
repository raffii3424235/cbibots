...
module.exports = {
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        if (!args[0]) return client.sendTime(message.channel, `🔉 | Volume saat ini \`${player.volume}\`.`);
        ...
        if (!parseInt(args[0])) return client.sendTime(message.channel, `**Pilih tingkat volume diantara** \`1 - 100\``);
     ...
     */
        run: async (client, interaction, args, { GuildDB }) => {
            ...
            if (!args[0].value) return client.sendTime(interaction, `🔉 | Volume saat ini \`${player.volume}\`.`);
            let vol = parseInt(args[0].value);
            if (!vol || vol < 1 || vol > 100) return client.sendTime(interaction, `**Pilih tingkat volume diantara** \`1 - 100\``);
            player.setVolume(vol);
